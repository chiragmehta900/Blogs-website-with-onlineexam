...
class Examination
{
	...
	{
		$this->host = 'localhost'; 	// Hosting name
		$this->username = 'root';  	// Hosting username
		$this->password = '';		// Hosting password
		$this->database = 'online_examination';	// Hosting database name
		$this->home_page = 'http://localhost/tutorial/online_examination/';  // past link ok server side this is for localhost
	...
	function send_email($receiver_email, $subject, $body)
	{
		...
		$mail->Host = 'smtp.hostinger.in'; 	// PHPMailer host link

		$mail->Port = '587';				// PHPMailer port number

		$mail->SMTPAuth = true;				// PHPMailer SMTPAuth ==> true or false

		$mail->Username = 'somnium_nostri@snproweb.com';	// PHPMailer username

		$mail->Password = 'Itsmechirag1';	// PHPMailer password

		$mail->SMTPSecure = 'TLS';			// PHPMailer SMTPSecure 

		$mail->From = 'somnium_nostri@snproweb.com';	// PHPMailer from gmail addresss

		$mail->FromName = 'Somnium_Nostri_Online_Examination';			// PHPMailer name from which mail is send
